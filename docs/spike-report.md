...
### 17.8 真機觀察 #3：C270 在暗處讀不準，疑似兩個新污染源（2026-09-09，使用者觀察）

使用者實際用 C270 跑一陣子後回報：**暗處分辨不出來**——關燈後的讀值仍偏高、判不到「暗（無光）」。兩個懷疑的污染源：

1. **螢幕背光散射**：C270 對微弱光較敏感（或自動曝光更積極），關燈時螢幕自身的背光散射足以把讀值頂到「暗」門檻以上。這是 §14 記過的結構性問題（螢幕自己的光污染判定）在新相機上更明顯的版本。
2. **相機自身的動作指示燈**：C270 鏡頭旁有一顆綠色指示燈，會「緩慢閃爍」。這個閃爍週期很可能就是 Lazy 取樣節奏本身——每次 Open（約 1.2s 取樣窗）燈亮、Release 後燈滅，加上快模式的 500ms 週期 → 看起來像不規則慢閃。若這顆燈的光反射回感光元件（例如相機朝牆放、§14.8 那種擺法，或桌面反光），就是**取樣行為自己造成的污染**，而且暗處相對佔比更大。

**留待重新校正時做的區辨實驗**（都很便宜）：

- 用不透光膠帶貼住 C270 的指示燈 → 重量關燈讀值。降下來 → 指示燈是因素之一。
- 關掉螢幕（或相機轉開、不對螢幕）→ 重量關燈讀值。降下來 → 螢幕背光散射是因素之一。
- 兩個都做 → 得到「純環境」的暗基準，才是新三段門檻的錨點。
...
**五條件實測（2026-09-09，`--sensitivity-probe 12`）**：
...
| 關燈・相機轉開 | 0.024 | 0.003 |
| 微光 / 一盞 | 0.149 | 0.035 |
| 一般開燈（暖機後）| 0.4495 | 0.002 |
| 一般開燈（剛開燈）| 0.439 | **0.218**（0.60→0.38，AGC 往下追）|
| 關燈・**相機朝亮螢幕** | 0.188 | 0.178（廢，同時蓋微光與開燈）|

- C270 感光遠優於舊機：暗處 ~0.02 而非 ~0.0005，舊「暗 <0.01」門檻失效。
- 螢幕只有開著才污染（螢幕關掉後相機朝不朝它沒差）。相機必須角度轉開不對螢幕。
- Lazy + 自動曝光在光線轉換時單一條件內就跳 0.2 → 正式校正要鎖曝光（C270 legacy Exposure 可鎖）。
- **已套用暫定門檻**（commit `b600dc0`）：暗 `<0.06`、微光 `<0.28`、開燈其餘。改了 `DefaultBands` + 本機 `config.json` + SelfTest（37/37）。正式校正待鎖曝光重做。
- 使用者提議把校正做成設定內自助流程 → issue #16。
