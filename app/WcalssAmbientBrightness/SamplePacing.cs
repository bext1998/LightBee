...
/// <summary>
/// 自適應取樣間隔：讀值穩定時維持慢間隔（預設 5 秒）；讀值大幅變化（|raw − 平滑值| ≥ deltaThreshold），
/// 或變動達半門檻且逼近任何分級邊界（距離 ≤ boundaryMargin）時切快間隔，讓「以取樣次數計」的防抖
/// 機制在時間軸上自動縮短——防抖的次數門檻不變，只是取樣密度跟著風險提高。
///
/// 快模式有取樣次數上限（超過強制一輪慢取樣），取樣失敗立刻退回慢間隔：高頻相機開關可能讓
/// FrameServer 卡死（spike-report §13.4），不對疑似故障的相機連續操作。
/// </summary>
internal sealed class SamplePacing
{
    ...
    public void OnSample(bool success, double raw, double smoothed)
    {
        ...
        var isMoving = Math.Abs(raw - smoothed) >= deltaThreshold * 0.5;
        var shouldFast = Math.Abs(raw - smoothed) >= deltaThreshold
            || (isMoving && (IsNearBoundary(raw) || IsNearBoundary(smoothed)));
        if (!shouldFast)
        {
            IsFastMode = false;
            fastCyclesUsed = 0;
